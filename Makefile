...
node: dir  ## Generates node resources
	...
	for filename in $(shell find api -iname "*.proto") ; do \
		protoc --proto_path=api --plugin=protoc-gen-ts=./node/node_modules/.bin/protoc-gen-ts --plugin=protoc-gen-grpc=./node/node_modules/.bin/grpc_tools_node_protoc_plugin --js_out=import_style=commonjs,binary:node --ts_out=service=grpc-node:node --grpc_out=node $$filename ; \
	done
...
clean: ## Deletes all generated files
	...
	rm -R `ls -d --ignore='node/node_modules' node/*/` || true
